<div>
    ...
    <script>
        function printQr(tableName, url) {
            ...
            @php
                $tenantName = auth()->user()->tenant->name ?? \App\Models\Setting::value('store_name', 'KedaiRobby.id');
                $tenantLogo = auth()->user()->tenant->logo ?? \App\Models\Setting::value('store_logo');
                $logoUrl = $tenantLogo ? asset('storage/' . $tenantLogo) : '';
            @endphp
            var logoUrl = @js($logoUrl);
            var storeName = @js($tenantName);

            // 4. Isi Konten Kartu
            printWindow.document.write(`
                <div class="card">
                    ${logoUrl ? `<div style="margin-bottom: 15px;"><img src="${logoUrl}" style="max-height: 55px; max-width: 140px; object-fit: contain; margin: 0 auto; display: block;"></div>` : ''}
                    <h1>SCAN HERE</h1>
                    ...
                    <div class="footer">POWERED BY ${storeName.toUpperCase()}</div>
                </div>
            `);
