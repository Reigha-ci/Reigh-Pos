        ...
        <form wire:submit="updateSettings" class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nomor WhatsApp Katering</label>
                        <input wire:model="cateringWhatsapp" type="text" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 font-medium text-slate-800 transition-all" placeholder="0812... (Tujuan konfirmasi katering)">
                        <p class="text-[10px] text-slate-400 mt-1">Nomor WhatsApp tujuan yang menerima pesan otomatis katering dari pelanggan.</p>
                        @error('cateringWhatsapp') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Minimal DP Katering (%)</label>
                        <div class="relative">
                            <input wire:model="cateringMinDp" type="number" min="10" max="100" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 font-medium text-slate-800 transition-all pr-12">
                            <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                                <span class="text-slate-500 font-bold">%</span>
                            </div>
                        </div>
                        <p class="text-[10px] text-slate-400 mt-1">Batas minimum uang muka katering (misal: 30% atau 50%).</p>
                        @error('cateringMinDp') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="pt-8 border-t border-slate-100">
                <h3 class="font-bold text-slate-700 border-b border-slate-100 pb-2 mb-6 uppercase text-xs tracking-wider">Rekening Bank & QRIS Pembayaran (Meja, Kios & Katering)</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Nama Bank</label>
                            <input wire:model="bankName" type="text" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 font-medium text-slate-800" placeholder="Contoh: Bank BCA, Mandiri, BRI">
                            @error('bankName') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Nomor Rekening</label>
                            <input wire:model="bankAccountNumber" type="text" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 font-medium text-slate-800" placeholder="Contoh: 1234567890">
                            @error('bankAccountNumber') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Atas Nama (A/N)</label>
                            <input wire:model="bankAccountName" type="text" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 font-medium text-slate-800" placeholder="Contoh: a.n. Kedai Robby">
                            @error('bankAccountName') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Foto / Barcode QRIS Toko</label>
                        <div class="flex items-start gap-4">
                            @if ($qrisImage && method_exists($qrisImage, 'temporaryUrl'))
                                <img src="{{ $qrisImage->temporaryUrl() }}" class="w-32 h-32 rounded-xl object-contain border border-slate-200 bg-slate-50 p-1">
                            @elseif ($existingQrisImage)
                                <img src="{{ asset('storage/' . $existingQrisImage) }}" class="w-32 h-32 rounded-xl object-contain border border-slate-200 bg-slate-50 p-1">
                            @elseif(file_exists(public_path('qris.jpg')))
                                <img src="{{ asset('qris.jpg') }}" class="w-32 h-32 rounded-xl object-contain border border-slate-200 bg-slate-50 p-1">
                            @else
                                <div class="w-32 h-32 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 text-center text-xs p-2">
                                    Belum ada QRIS
                                </div>
                            @endif

                            <div class="flex-1">
                                <input wire:model="qrisImage" type="file" accept="image/*" class="block w-full text-sm text-slate-500 rounded-xl border border-slate-200 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-all">
                                <p class="text-[10px] text-slate-400 mt-1">Upload QRIS statis toko (JPG, PNG max 2MB). QRIS ini akan tampil otomatis di modal bayar QRIS meja, kios, dan invoice katering.</p>
                                @error('qrisImage') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                </div>
