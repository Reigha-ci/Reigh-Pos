<div class="p-6"> 
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex items-center gap-4 w-full md:w-auto">
            <div class="relative flex-1 md:w-64">
                <input wire:model.live="search" type="text" placeholder="Cari Pelanggan / ID..." class="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                <div class="absolute left-3 top-2.5 text-slate-400">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
            </div>

            <div wire:poll.10s class="hidden sm:flex text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full items-center gap-2 border border-green-100 whitespace-nowrap">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        ...
                        @if($order->payment_method == 'qris' && $order->status != 'paid')
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200 mt-1 animate-pulse">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 14.5V12m0 0V8.882c0-1.126.965-1.956 1.99-1.637l.01.002.01.003c.125.04.25.07.376.096A4.953 4.953 0 0118 10.686V13m-2.222-7.5a4.956 4.956 0 00-4.556 0M10.889 5.5a4.956 4.956 0 00-4.556 m2.222 7.5V13m0-2.314v-2.11c0-1.29 1.155-2.27 2.433-2.09A4.95 4.95 0 0110 10.686v2.314"></path></svg>
                                Cek Mutasi (QRIS)
                            </span>
                    ...
                    <td class="px-6 py-4 text-center flex items-center justify-center gap-2">
                        @if($order->status == 'paid')
                            <button wire:click="markAsCompleted({{ $order->id }})" class="bg-emerald-500 text-white p-2 rounded-xl hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 transition-all active:scale-95" title="Selesaikan & Arsip">
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-zoom-in flex flex-col max-h-[90vh]">
                    ...
                    @if($selectedOrder->status == 'paid' || $selectedOrder->status == 'completed')
                        <div class="text-center py-10 flex flex-col justify-center h-full">
                            ...
                            <div class="grid grid-cols-2 gap-3 mb-3">
                                ...
                                <button wire:click="printDirect({{ $selectedOrder->id }})" wire:loading.attr="disabled" class="w-full bg-slate-800 text-white py-3 rounded-xl font-bold hover:bg-slate-900 flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 transition-all active:scale-95">
