            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-xs font-medium text-slate-500">{{ $customer->last_visit ? \Carbon\Carbon::parse($customer->last_visit)->diffForHumans() : '-' }}</span>
                        </td>
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center px-4">
        ...
        <div class="bg-white w-full max-w-2xl rounded-[2.5rem] relative shadow-2xl animate-scale-in flex flex-col overflow-hidden max-h-[85vh]">
            ...
            <div class="p-6 border-t border-slate-100 flex justify-center bg-slate-50/30">
                <span class="text-xs font-bold text-slate-500 italic">Loyalty Program {{ auth()->user()->tenant->name ?? \App\Models\Setting::value('store_name', 'KedaiRobby.id') }} • Real-time tracking</span>
            </div>
        </div>
    </div>
    @endif
