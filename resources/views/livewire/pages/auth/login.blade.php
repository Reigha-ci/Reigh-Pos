    ...
    <div class="hidden lg:flex relative items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div class="relative z-20 max-w-xl text-center p-16">
            <div class="mb-8 flex justify-center">
                @php
                    $globalName = \App\Models\Setting::value('store_name', config('app.name', 'KedaiRobby.id'));
                    $globalLogo = \App\Models\Setting::value('store_logo');
                @endphp
                <div class="w-24 h-24 rounded-3xl bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center shadow-2xl overflow-hidden p-3">
                    @if($globalLogo)
                        <img src="{{ asset('storage/' . $globalLogo) }}" alt="{{ $globalName }}" class="w-full h-full object-contain">
                    @else
                        <div class="w-full h-full flex items-center justify-center text-white font-black text-4xl">
                            {{ strtoupper(substr($globalName, 0, 1)) }}
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="mb-10 text-left">
                <div class="flex items-center gap-3 mb-8">
                    @if($globalLogo)
                        <img src="{{ asset('storage/' . $globalLogo) }}" alt="{{ $globalName }}" class="w-10 h-10 rounded-xl object-cover shadow-md border border-slate-100 bg-white">
                    @else
                        <div class="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white shadow-lg shadow-indigo-600/20 font-black text-lg">
                            {{ strtoupper(substr($globalName, 0, 1)) }}
                        </div>
                    @endif
                    <span class="text-2xl font-black text-slate-900 tracking-tight">{{ $globalName }}</span>
                </div>
            ...
            <div class="mt-8 text-center">
                 <p class="text-xs text-slate-400 font-medium">
                    © {{ date('Y') }} {{ $globalName }}. All rights reserved.
                </p>
            </div>
        </div>
    </div>
