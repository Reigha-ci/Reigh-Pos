...
<head>
    ...
    <title>{{ auth()->user()->tenant->name ?? \App\Models\Setting::value('store_name', config('app.name', 'KedaiRobby.id')) }} - Admin</title>

    <link rel="icon" href="{{ ($logo = (auth()->user()->tenant->logo ?? \App\Models\Setting::value('store_logo'))) ? asset('storage/' . $logo) : asset('favicon.svg') }}">
...
<body class="font-sans antialiased bg-[#F8FAFC] text-slate-900 selection:bg-indigo-500 selection:text-white" x-data="{ sidebarOpen: false }">

    @php
        $isSuperAdmin = auth()->user()->role === \App\Enums\UserRole::SUPER_ADMIN;
        $tenant = auth()->user()->tenant;
        $tenantName = $tenant->name ?? \App\Models\Setting::value('store_name', 'KedaiRobby.id');
        $tenantLogo = $tenant->logo ?? \App\Models\Setting::value('store_logo');
    @endphp

    <aside class="fixed inset-y-0 left-0 z-50 w-72 bg-[#0B0F17] border-r border-slate-800/60 flex flex-col transition-transform duration-300 ease-in-out sm:translate-x-0" 
           :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}">
        
        <div class="h-20 flex items-center px-6 border-b border-slate-800/50 bg-[#0F1623] shrink-0">
            <div class="flex items-center gap-3.5">
                ...
                @elseif($tenantLogo)
                    <img src="{{ asset('storage/' . $tenantLogo) }}" class="w-10 h-10 rounded-xl object-cover ring-1 ring-white/10 shadow-lg shadow-indigo-500/20">
                @else
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/20 ring-1 ring-white/10 text-white font-black text-base">
                        {{ strtoupper(substr($tenantName, 0, 1)) }}
                    </div>
                @endif
                <div>
                    <h1 class="text-xl font-black tracking-tight text-white leading-none truncate max-w-[140px]" title="{{ $isSuperAdmin ? 'Platform Admin' : $tenantName }}">
                        @if($isSuperAdmin)
                            Platform Admin
                        @else
                            {{ $tenantName }}
        ...
        <nav class="flex-1 overflow-y-auto py-8 px-5 space-y-10 custom-scrollbar">
            ...
            <div>
                ...
                <div class="space-y-2">
                    ...
                    @if(auth()->user()->hasRole(\App\Enums\UserRole::CASHIER) || auth()->user()->hasRole(\App\Enums\UserRole::OWNER) || auth()->user()->hasRole(\App\Enums\UserRole::KITCHEN))
                    <a wire:navigate href="{{ route('admin.catering') }}" class="flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('admin.catering') ? 'bg-amber-600 text-white shadow-lg shadow-amber-500/25 ring-1 ring-amber-400/20' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white' }}">
                        <div class="flex items-center gap-3.5">
                            <svg class="w-5 h-5 {{ request()->routeIs('admin.catering') ? 'text-white' : 'text-slate-500 group-hover:text-white' }} transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                            </svg>
                            <span class="font-bold text-sm">Pesanan Katering</span>
                        </div>
                        @php
                            $pendingCatering = \App\Models\Order::catering()->where('catering_status', 'pending')->count();
                        @endphp
                        @if($pendingCatering > 0)
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-500 text-white shadow-xs">
                                {{ $pendingCatering }}
                            </span>
                        @endif
                    </a>
