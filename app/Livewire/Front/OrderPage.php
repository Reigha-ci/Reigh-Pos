...
class OrderPage extends Component
{
    ...
    // Rekening & QRIS
    public $bankName = 'Bank BCA';
    public $bankAccountNumber = '123 456 7890';
    public $bankAccountName = 'a.n. Kedai Robby';
    ...
    {
        $this->table = Table::withoutGlobalScope('tenant')->where('slug', $slug)->firstOrFail();
        ...
        $this->bankName = \App\Models\Setting::value('bank_name', 'Bank BCA');
        $this->bankAccountNumber = \App\Models\Setting::value('bank_account_number', '123 456 7890');
        $this->bankAccountName = \App\Models\Setting::value('bank_account_name', 'a.n. Kedai Robby');
        $this->qrisImage = \App\Models\Setting::value('qris_image', null);
    }
    ...
    public function processCheckout()
    {
        $this->validate([
            ...
            'paymentMethod' => 'required|in:cashier,midtrans,qris,transfer',
            'memberPhone' => 'nullable|numeric|digits_between:10,15',
        ]);

        if (empty($this->cart)) return;

        if (in_array($this->paymentMethod, ['qris', 'transfer'])) {
